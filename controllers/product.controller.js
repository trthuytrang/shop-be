...
module.exports.postProduct = async function(req, res) {
	const imgArr = [];
	req.files.map((item)=>{
		imgArr.push(`localhost:4000/${item.path.split("/").slice(1).join("/")}`)
	})
	const data = {
	...
	for (let i in emailList) {
		...
		var mailOptions = {
			...
			`<img src="localhost:4000/email/${emailList[i]._id}/${emailInfo.sendedEmail[emailInfo.sendedEmail.length - 1].emailId}" alt=""></img>`
		}
